...
isflaskdir_exist = os.path.exists('requirements.txt')
if isflaskdir_exist:
    subprocess.call(['rm','-fr','flask'])
subprocess.call(['python2.7', 'virtualenv.py', 'flask'])
if sys.platform == 'win32':
    bin = 'Scripts'
else:
    bin = 'bin'
# subprocess.call(['source', os.path.join('flask', bin, 'activate')])
isexist_requirements = os.path.exists('requirements.txt')

if isexist_requirements:
    subprocess.call([os.path.join('flask', bin, 'pip'), 'install', '-r', 'requirements.txt'])
else:
    ...
    subprocess.call([os.path.join('flask', bin, 'pip'), 'install', 'guess-language-spirit'])
